...
class BaseTrainer(object):
    ...
    def log(self, lr, epoch, log_dict):
        ...
        if log_dict["test_precision"] > self.best_precision:
            self.best_precision = log_dict["test_precision"]
            if self.cfg.LOG.WANDB:
                wandb.run.summary["best_precision"] = self.best_precision
        if log_dict["test_recall"] > self.best_recall:
            self.best_recall = log_dict["test_recall"]
            if self.cfg.LOG.WANDB:
                wandb.run.summary["best_recall"] = self.best_recall
        if log_dict["test_f1"] > self.best_f1:
            self.best_f1 = log_dict["test_f1"]
            if self.cfg.LOG.WANDB:
        ...
        with open(os.path.join(self.log_path, "worklog.txt"), "a") as writer:
            ...
            for k, v in log_dict.items():
                if isinstance(v, str):  # 检查 v 是否为字符串
                    lines.append("{}: {}".format(k, v) + os.linesep)  # 字符串直接格式化
                else:
    ...
    def train(self, resume=False):
        ...
        print(log_msg("Best precision:{}".format(self.best_precision), "EVAL"))
        print(log_msg("Best recall:{}".format(self.best_recall), "EVAL"))
        print(log_msg("Best f1:{}".format(self.best_f1), "EVAL"))
        with open(os.path.join(self.log_path, "worklog.txt"), "a") as writer:
            writer.write("best_acc\t" + "{:.2f}".format(float(self.best_acc)))
            writer.write("best_precision\t" + "{:.2f}".format(float(self.best_precision)))
            writer.write("best_recall\t" + "{:.2f}".format(float(self.best_recall)))
            writer.write("best_f1\t" + "{:.2f}".format(float(self.best_f1)))
    ...
    def train_epoch(self, epoch):
        ...
        # train loops
        self.distiller.train() #学生模型设置成训练模式，教师模型设置成验证模式
        ...
        # validate
        test_acc, test_acc_top5, test_loss, precision, recall, f1, res= validate(self.val_loader, self.distiller)
        flattened_res = [item for sublist in res for item in sublist]
        res = ','.join(map(str, flattened_res))
