...
class Res2NetBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, downsample=None, stride=1, scales=4, groups=1, se=False,  norm_layer=None,is_last=False):
...
class CifarRes2Net(nn.Module):
    def __init__(self, layers, num_classes=100, zero_init_residual=False,
        ...
        planes = [int(width * scales * 2 ** i) for i in range(3)] #256,512,1024
    ...
    def _make_layer(self, block, planes, blocks, stride=1, scales=4, groups=1, se=False, norm_layer=None):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, downsample, stride=stride, scales=scales, groups=groups, se=se, norm_layer=norm_layer,is_last=(blocks == 1)))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, scales=scales, groups=groups, se=se, norm_layer=norm_layer,is_last=(i == blocks - 1)))
...
def res2next29_8cx24wx4scale(**kwargs):
    """Constructs a Res2NeXt-29, 6cx24wx4scale model.
    """
    model = CifarRes2Net([3, 3, 3], groups=8, width=24, scales=4, **kwargs)
    return model
...
def res2next29_6cx24wx6scale_se_min(**kwargs):
    """Constructs a Res2NeXt-29, 6cx24wx6scale-SE model.
    """
    model = CifarRes2Net([1, 1, 1], groups=6, width=24, scales=6, se=True, **kwargs)
    return model
...
if __name__ == "__main__":
    ...
    net = res2next29_6cx24wx6scale_se(num_classes=20)
    logit, feats = net(x)
