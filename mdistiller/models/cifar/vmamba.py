...
class Linear2d(nn.Linear):
    def forward(self, x: torch.Tensor):
        # B, C, H, W = x.shape
        x = x.cuda(device='cuda:0',non_blocking=True)
...
class LayerNorm2d(nn.LayerNorm):
    def forward(self, x: torch.Tensor):
        x = x.permute(0, 2, 3, 1)
        x = x.cuda(device=0, non_blocking=True)
    ...
    def forward_corev2(
        ...
        if cascade2d:
            ...
            if no_einsum:
                ...
                x_proj_weight = x_proj_weight.cuda(non_blocking=True)
                ...
                dts = dts.cuda(non_blocking=True)
                dt_projs_weight = dt_projs_weight.cuda(non_blocking=True)
...
class VSSM(nn.Module):
    def __init__(
        ...
        self.norm=norm_layer(self.num_features)  # B,H,W,C
    ...
    def forward(self, x: torch.Tensor):
        x = x.to(torch.device('cuda:0'), non_blocking=True)
        ...
        f0 = f0.permute(0, 3, 2, 1)  # (batch_size, height * width, num_features)
        f1 = f1.permute(0, 3, 2, 1)
        f2 = f2.permute(0, 3, 2, 1)
...
def vmamba_small(num_classes: int = 5, **kwargs):
...
def vmamba_base(num_classes: int = 5, **kwargs):
    model = VSSM(num_classes=num_classes,
                 drop_path_rate=0.6,
                 dims=128,
                 depths=[2,2,15,2],
