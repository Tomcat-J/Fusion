...
class VID(Distiller):
    ...
    def __init__(self, student, teacher, cfg):
        ...
        feat_s_shapes, feat_t_shapes = get_feat_shapes(
            self.student, self.teacher, cfg.VID.INPUT_SIZE #student和tearcher中feats["feats"]的输出体积大小
        )
        feat_s_channels = [s[1] for s in feat_s_shapes[1:]]#除了f0.f1,f2,f3的通道数
    ...
    def init_vid_modules(self, feat_s_shapes, feat_t_shapes):
        ...
        for s, t in zip(feat_s_shapes, feat_t_shapes):
            ...
            )#[ln(e(5-1e-5)-1),ln(e(5-1e-5)-1),ln(e(5-1e-5)-1),...]可训练
            self.log_scales.append(log_scale)

    def get_learnable_parameters(self):
        parameters = super().get_learnable_parameters()#返回学生网络每一层的参数
        for regressor in self.regressors:
            parameters += list(regressor.parameters())#连接起来
        return parameters

    def get_extra_parameters(self): #返回regressor参数总量的值
        num_p = 0
        for regressor in self.regressors:
