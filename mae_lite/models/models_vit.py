        ...
        if self.global_pool:
            ...
            if self.distilled:
                ...
                if self.num_classes > 0:
                    # self.head_dist = Head(self.num_classes, self.embed_dim,self.num_heads,self.embed_dim)
                    self.head_dist = nn.Linear(self.embed_dim, self.num_classes)
                else:
                    self.head_dist = nn.Identity()
        self.head = Head(self.num_classes, self.embed_dim,self.num_heads,self.embed_dim) if num_classes > 0 else nn.Identity()
        self.init_weights("")
